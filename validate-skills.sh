...
for skill_dir in "$SKILLS_DIR"/*/; do
    ...
    # ===== NAME VALIDATION =====
    name_in_file=$(echo "$frontmatter" | grep "^name:" | sed 's/^name: //' | tr -d ' ' | tr -d '"' | tr -d "'")
    ...
    if [[ -z "$description" ]]; then
        ...
        if ! echo "$description" | grep -qi "when\|mention\|use\|trigger\|apply"; then
            skill_warnings+=("Description lacks clear trigger phrases ('when', 'mention', 'use', 'apply', 'trigger')")
        fi
    ...
    license=$(echo "$frontmatter" | grep "^license:" | sed 's/^license: //' | tr -d ' ' | tr -d '"' | tr -d "'")
